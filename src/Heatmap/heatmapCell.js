...
const HeatMapHighlightCell = ({ data }) => {
  ...
  const highlightedRects = data.map((d, i) => {
    ...
    const contrastColor = d3.lab(colorScale(d.value)).l > 5 ? '#000000' : '#ffffff';
    const cellColor = d.x === allXGroups[3] && d.y === allYGroups[4] ? colorScale(d.value) : '#ffffff'; // Highlight the specified cell
...
export default HeatMapHighlightCell;
